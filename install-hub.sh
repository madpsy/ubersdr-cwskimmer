...
" <<< "$json" 2>/dev/null
    ...
    # This ensures all config vars (including future additions) appear in .env
    # without needing to update the heredoc here.
    sed \
        -e "s|^CALLSIGN=.*|CALLSIGN=${CALLSIGN}|" \
        -e "s|^NAME=.*|NAME=${NAME}|" \
        -e "s|^QTH=.*|QTH=${QTH}|" \
        -e "s|^SQUARE=.*|SQUARE=${SQUARE}|" \
        -e "s|^UBERSDR_HOST=.*|UBERSDR_HOST=${UBERSDR_HOST}|" \
        -e "s|^UBERSDR_PORT=.*|UBERSDR_PORT=${UBERSDR_PORT}|" \
        -e "s|^RBN_SEND_SPOTS=.*|RBN_SEND_SPOTS=${RBN_SEND_SPOTS}|" \
        -e "s|^BAND_160M=.*|BAND_160M=${BAND_160M}|" \
        -e "s|^BAND_80M=.*|BAND_80M=${BAND_80M}|" \
        -e "s|^BAND_60M=.*|BAND_60M=${BAND_60M}|" \
        -e "s|^BAND_40M=.*|BAND_40M=${BAND_40M}|" \
        -e "s|^BAND_30M=.*|BAND_30M=${BAND_30M}|" \
        -e "s|^BAND_20M=.*|BAND_20M=${BAND_20M}|" \
        -e "s|^BAND_17M=.*|BAND_17M=${BAND_17M}|" \
        -e "s|^BAND_15M=.*|BAND_15M=${BAND_15M}|" \
